...
프론트엔드 과정에서 알고리즘 설계자가 알고리즘을 정하거나 설계하면 그것을 float모델로 만들어 정확한 실수값들의 연산으로 알고리즘을 확인해가며 원하는 출력을 만들어 내는지 확인 합니다.
이때 Fixed모델은 완성된 float모델을 고정 소수점을 사용하여 정수값들의 연산으로 수정한 모델로 이러한 과정은 비트들로 이루어진 연산을 수행하는 하드웨어에 있어서 최적화된 모델을 만들기 위함 입니다.
고정 소수점으로 실수값들을 정수로 나타낼때는 부호비트와 정수비트 그리고 소수비트로 구성됩니다. 소수비트값들의 경우
...
## (5) FPGA Targeting
<img src="/History/img/img1.png" width=150 height=150>
### Vivado
